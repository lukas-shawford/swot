    ...
    return promise
        ...
        .then(function (topic) {
            ...
            if (data.parent) {
                ...
                return parentTopic.saveQ().then(function () {
                    return user;
                });
            } else {
...
/**
 * Updates the given topic's position in the parent's subtopics list (or in the case of root topics,
 * moves the topic to the appropriate position in the user.topics array).
 * @param topic The topic document whose position should be changed
 * @param position The position (starting at 0) to move the topic to.
 * @return {Promise}
 */
exports.updateTopicPosition = function (topic, position) {
    if (topic.parent) {
        return Q(Topic.findById(topic.parent).exec())
            .then(function (parent) {
                var oldPos = parent.subtopics.indexOf(topic._id);
                if (oldPos === -1) {
                    throw new Error("Topic \"" + topic.name + "\" (" + topic._id.toString() +
                        ") is not a subtopic of \"" + parent.name + "\" (" + parent._id.toString() + ").");
                }
                parent.subtopics.splice(position, 0, parent.subtopics.splice(oldPos, 1)[0]);
                return parent.saveQ();
            });
    } else {
        return Q(User.findById(topic.createdBy).exec())
            .then(function (user) {
                var oldPos = user.topics.indexOf(topic._id);
                if (oldPos === -1) {
                    throw new Error("Topic \"" + topic.name + "\" (" + topic._id.toString() +
                        ") has no parent (indicating that it is a root topic), but it could not be " +
                        "found in the user.topics array.");
                }
                user.topics.splice(position, 0, user.topics.splice(oldPos, 1)[0]);
                return user.saveQ();
            });
    }
};
...
/**
 * Moves the given topic to a different parent topic.
 * @param topic The topic to move.
 * @param parent The new parent topic. If null, the topic will be made into a root-level topic and
 *      added to the user.topics array.
 * @param position The position that the topic should be moved to with the new parent's subtopics
 *      array (or the user.topics array if the topic is now a root topic). If not passed in, the
 *      topic will be appended at the end of the list.
 */
exports.moveTopic = function (topic, parent, position) {
    return Q()
        .then(function () {
            // Verify parent topic exists and is owned by the same user before changing anything.
            if (parent) {
                return Q(Topic.findById(parent._id).exec())
                    .then(function (parentTopic) {
                        if (!parentTopic) {
                            throw new Error("Topic not found.");
                        }
                        if (!parentTopic.createdBy.equals(topic.createdBy)) {
        ...
        .then(function () {
            // Remove the topic from the current parent's subtopics array (or the user.topics array if the
            // topic is currently a root-level topic).
            if (topic.parent) { // Topic is currently a subtopic
                return Q(Topic.findById(topic.parent).exec())
                    .then(function (oldParent) {
                        oldParent.subtopics.pull(topic._id);
                        return oldParent.saveQ();
                    });
            } else {    // Topic is currently a root topic
                return Q(User.findById(topic.createdBy).exec())
                    .then(function (user) {
        ...
        .then(function () {
            // Update the topic's parent
            topic.parent = parent;
            return topic.saveQ();
        })
        .then(function (topic) {
            // Add the topic to the new parent topic's subtopics array (or user.topics if it is now a root topic).
            if (parent) {
                // Add subtopics to the parent topic's subtopics array
                parent.subtopics.push(topic);
                //return Q.ninvoke(parent, "save");
                return parent.saveQ();
            } else {
                // Add top-level topics to user.topics
                return Q(User.findById(topic.createdBy).exec())
        ...
        .then(function () {
            // Update the position (if specified)
            if (position !== undefined) {
                return exports.updateTopicPosition(topic, position);
            }
        });
};
